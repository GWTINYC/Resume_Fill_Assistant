...
try{
 ...
 const degree2=field('学历','教育经历',2);await page.locator('#section-2 .record').nth(1).locator('[role=option]').nth(1).evaluate(el=>el.textContent='选项变更');const stale=await page.evaluate(pageBridge,{action:'fill',token:'test',items:[{id:degree2.id,value:'硕士'}]});assert.equal(stale.results[0].ok,false);assert.match(stale.results[0].reason,/SELECT_NO_MATCH/);
