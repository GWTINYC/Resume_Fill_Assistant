...
let entries=[],fields=[],plan=new Map(),tabId,token,profileSnapshot='',materialSnapshot='',savedProfile={},materials=[],provider='jev',busy=false,pageContext={},collaborationController=null;
...
async function scanCurrent(resetOverwrite=true){
 ...
 const r=await message({action:'scan',tabId:tab.id});tabId=tab.id;token=r.token;fields=r.fields;pageContext=r.pageContext||{};plan.clear();if(resetOverwrite)$('overwrite').checked=false;
...
$('collaborate').onclick=async()=>{
 ...
 try{
  ...
  const count=state=>result.filter(x=>x.status===state).length;status(`协作完成：已填入并核验 ${count('filled')} 项，待人工核对 ${count('needs_review')} 项，填写失败 ${count('failed')} 项，跳过 ${count('skipped')} 项。已保留出处与校核结果，请检查后自行提交。`);
