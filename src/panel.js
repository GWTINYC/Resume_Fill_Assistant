...
let entries=[],fields=[],plan=new Map(),tabId,token,profileSnapshot='',materialSnapshot='',savedProfile={},materials=[],provider='jev',busy=false;
const usage={jev:0,deepseekInput:0,deepseekOutput:0};
function status(text,error=false){$('status').textContent=text;$('status').classList.toggle('error',error);}
function lock(value){busy=value;for(const id of ['scan','match','fill'])$(id).disabled=value||(id!=='scan'&&!fields.length);for(const id of ['provider','save-key','clear-key','overwrite'])$(id).disabled=value;for(const el of $('fields').querySelectorAll('select,input,textarea'))el.disabled=value||el.dataset.unsupported==='true';}
...
async function keyStatus(){const apiKey=await getApiKey(provider);$('key-info').textContent=apiKey?'已持久保存':'未设置';}
function providerUI(){
...
function render(){
 ...
 for(const f of fields){
  ...
  if(p.ai&&f.supported){
    ...
    editor.value=p.value??'';editor.onchange=()=>{p.value=defaultFieldValue(f,editor.value);p.source='手动修改 DeepSeek 建议';p.edited=true;if(p.value===null||p.value===''){p.value=null;p.checked=false;}render();};card.append(editor);
...
$('scan').onclick=async()=>{lock(true);try{
 ...
 status(fields.length?`扫描完成。核对本地匹配，或点击当前服务的智能识别按钮。${r.atLimit?' 单个框架最多扫描 100 项，请分步处理。':''}${r.limited?' 部分框架不可访问。':''}`:'没有找到可见表单。自定义控件、跨域框架或 Shadow DOM 可能需要网站专用适配。');render();
