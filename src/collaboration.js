 ...
 for(let i=0;i<proposals.length;i+=2){
  ...
  for(const [chunkIndex,sourcePortion]of chunks.entries()){
   ...
   for(let j=0;j<items.length;j++){
    ...
    questions[`q${j}_unsupported`]={type:'noul',instructions:prefix+'Does proposedValue depart from the appropriate source text or distort it by incomplete extraction? Text must copy the matching original value or complete prepared passage verbatim. Reject paraphrasing, translation, synthesis, omitted qualifications/negations, fragments that change meaning, or a passage shortened solely to fit maxLength. Preserve original punctuation and wording. Only native date/month format conversion and semantically equivalent fixed options may differ in representation. Do not allow inferred skills or new motivation even if plausible.',criteria:{true:'The answer rewrites, adds to, or misleadingly truncates the original material.',false:'The matching original value or complete passage is faithfully copied, with only the allowed control-format exceptions.'}};
