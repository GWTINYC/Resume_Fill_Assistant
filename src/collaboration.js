...
export const REVIEW_THRESHOLDS={fact:{fit:.9,grounded:.9,consistent:.9},expression:{fit:.85,grounded:.85,consistent:.9}};
...
export function auditPackets(proposals,fields,sources,pageContext={}){
 ...
 for(let i=0;i<proposals.length;i+=2){
  const pair=proposals.slice(i,i+2);const items=pair.map(p=>{const f=byId.get(p.fieldId);return {field:{id:f.id,label:f.label,type:f.type,context:f.context,placeholder:f.placeholder,maxLength:f.maxLength,options:f.options},proposedValue:p.value,evidence:evidenceContext(p,sources)};});
  for(const [chunkIndex,sourcePortion]of chunks.entries()){
   ...
   for(let j=0;j<items.length;j++){
    ...
    questions[`q${j}_policy`]={type:'choice',instructions:prefix+'What kind of answer is requested by field.label and field.context?',criteria:{fact:'A concrete personal fact or a fixed-option selection, such as name, date, degree, employer, salary or eligibility.',expression:'Open-ended wording, such as an introduction, motivation, project description or strengths statement.',unclear:'Unclear field meaning or record identity, or a consent/declaration that should not be filled.'}};
    questions[`q${j}_fit`]={type:'noul',instructions:prefix+'Does proposedValue address the subject requested by field.label and field.context? For an introduction or description, a relevant summary fits: it need not be a full biography or copy the source wording. An answer from the wrong employment or education record does not fit.',criteria:{true:'The answer is relevant to the requested field and the correct record.',false:'The answer concerns a different subject or record, or the mapping is ambiguous.'}};
    questions[`q${j}_unsupported`]={type:'noul',instructions:prefix+'Does proposedValue add an unsupported concrete fact about the applicant? Check against evidence and its context. New employers, dates, credentials, achievements, metrics, expertise levels, salary, availability or legal commitments need evidence. Faithful paraphrase, modest skills demonstrated by an experience, and prospective motivation connecting that experience to the role are allowed; do not judge writing style.',criteria:{true:'At least one concrete claim is invented, inflated or unsupported.',false:'No unsupported concrete claim; flexible wording stays within the supplied facts.'}};
   ...
   if(size(payload.state)>23000)throw Error('字段或引用过长，无法完整校核，请缩短启用素材或先整理为资料项。');
...
export function readAudit(packet,answers){
 return packet.ids.map((fieldId,i)=>({fieldId,policy:acceptedChoice(answers?.[`q${i}_policy`],['fact','expression']),fit:safeNoul(answers?.[`q${i}_fit`]),grounded:safeNoul(answers?.[`q${i}_unsupported`])===null?null:1-safeNoul(answers[`q${i}_unsupported`]),consistent:safeNoul(answers?.[`q${i}_conflict`])===null?null:1-safeNoul(answers[`q${i}_conflict`])}));
}
...
export function localConcerns(field,proposal,entries){
 const reasons=[];const id=localMapping(field,entries);const entry=entries.find(e=>e.id===id);
...
export function combineAudit(field,proposal,entries,checks){
 ...
 for(const [key,label]of [['fit','字段与经历对应'],['grounded','事实依据与表达尺度'],['consistent','跨素材一致性']])if(!checks.length||checks.some(x=>x[key]===null||x[key]<thresholds[key]))reasons.push(`${label}未通过校核。`);
...
export async function runCollaboration({fields,sources,entries,pageContext={},draft,judge,apply,assertFresh,onProgress=()=>{},signal,overwrite=false}){
 ...
 for(let round=0;round<2&&pending.length;round++){
  ...
  for(let i=0;i<pending.length;i+=6){
   checkSignal(signal);const batch=pending.slice(i,i+6);emit(round?'repair':'draft',`${round?'DeepSeek 正在修正':'DeepSeek 正在理解页面与拟定答案'} ${i+1}–${Math.min(i+6,pending.length)} / ${pending.length}`);
   ...
   for(const f of batch){const record=records.get(f.id),proposal=returned.get(f.id);if(!proposal){record.status='needs_review';record.reason=response.issues?.find(x=>x.fieldId===f.id)?.reason||'没有得到具备可验证出处的答案；请补充资料或手动填写。';record.review=null;record.proposal=null;continue;}record.proposal=proposal;record.status='checking';record.reason='等待 Jev 独立校核';candidates.push(proposal);}
  }
